...
if __name__ == "__main__":
    ...
    '''Pruning SAE - trained by MATS'''
    gpt2: sae_lens.HookedSAETransformer = sae_lens.HookedSAETransformer.from_pretrained("gpt2-small", device=device)

    release = "gpt2-small-hook-z-kk"
    sae_id = "blocks.9.hook_z"
    gpt2_sae = sae_lens.SAE.from_pretrained(release, sae_id, device=str(device))[0]

    for batch in tqdm(openwebtext):
        gpt2_sae = prune_sae(gpt2_sae, batch)
    
    t.save(gpt2_sae.state_dict(), 'pruned_gpt2_attn_sae_wanda.pth')
