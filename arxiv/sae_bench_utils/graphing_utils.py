...
TRAINER_LABELS = {
    "pretrained": "Pretrained SAE",
    "trained": "SAE trained on pruned model",
    "pruned25": "Pruned SAE with 0.25 sparsity",
    "pruned50": "Pruned SAE with 0.50 sparsity"
}
...
def get_custom_metric_key_and_name(
    ...
    print("DEBUG: get_custom_metric_key_and_name", eval_path, k)
...
def get_eval_results(eval_filenames: list[str]) -> dict[str, dict]:
    ...
    for filepath in eval_filenames:
        ...
        # eval_results[results_key] = filepath.split("/local/scratch/suchit/COLM", 1)[-1]

        if "tpp" in filepath.split("/local/scratch/suchit/COLM", 1)[-1]:
            eval_results[results_key] = single_sae_results["eval_result_metrics"][
                "tpp_metrics"
            ]
        elif "scr" in filepath.split("/local/scratch/suchit/COLM", 1)[-1]:
            eval_results[results_key] = single_sae_results["eval_result_metrics"][
                "scr_metrics"
            ]
        elif "absorption" in filepath.split("/local/scratch/suchit/COLM", 1)[-1]:
            eval_results[results_key] = single_sae_results["eval_result_metrics"][
                "mean"
            ]
        elif "autointerp" in filepath.split("/local/scratch/suchit/COLM", 1)[-1]:
            eval_results[results_key] = single_sae_results["eval_result_metrics"][
                "autointerp"
            ]
        elif "sparse_probing" in filepath.split("/local/scratch/suchit/COLM", 1)[-1]:
            eval_results[results_key] = single_sae_results["eval_result_metrics"]["sae"]
        elif "unlearning" in filepath:
            eval_results[results_key] = single_sae_results["eval_result_metrics"][
                "unlearning"
            ]
        elif "core" in filepath.split("/local/scratch/suchit/COLM", 1)[-1]:
        ...
        # print("canonical" in filepath.split("/local/scratch/suchit/COLM", 1)[-1])
        # print("pile-0.25" in filepath.split("/local/scratch/suchit/COLM", 1)[-1])
        # print("pile-0.5" in filepath.split("/local/scratch/suchit/COLM", 1)[-1])
        # print(filepath.split("/local/scratch/suchit/COLM/test", 1)[-1])

        # GPT2
        # if "32k" in filepath.split("/local/scratch/suchit/COLM/test", 1)[-1]:
        #     eval_results[results_key]["sae_class"] = "pretrained"
        # elif "openwebtext_0.25" in filepath.split("/local/scratch/suchit/COLM/test", 1)[-1]:
        #     eval_results[results_key]["sae_class"] = "pruned25"
        # elif "openwebtext_0.5" in filepath.split("/local/scratch/suchit/COLM/test", 1)[-1]:
        #     eval_results[results_key]["sae_class"] = "pruned50"
        # elif "openwebtext-wanda" in filepath.split("/local/scratch/suchit/COLM/test", 1)[-1]:
        #     eval_results[results_key]["sae_class"] = "trained"
        # elif "openwebtext_blocks" in filepath.split("/local/scratch/suchit/COLM/test", 1)[-1]:
        #     eval_results[results_key]["sae_class"] = "prunedBest"
        # else:
        #     raise ValueError("Please add the correct key for the eval results")

        # Gemma-2-2b
        if "canonical" in filepath.split("/local/scratch/suchit/COLM/eval_results", 1)[-1]:
            eval_results[results_key]["sae_class"] = "pretrained"
        elif "pile-0.25" in filepath.split("/local/scratch/suchit/COLM/eval_results", 1)[-1]:
            eval_results[results_key]["sae_class"] = "pruned25"
        elif "pile-0.5" in filepath.split("/local/scratch/suchit/COLM/eval_results", 1)[-1]:
            eval_results[results_key]["sae_class"] = "pruned50"
        elif "pile-wanda" in filepath.split("/local/scratch/suchit/COLM/eval_results", 1)[-1]:
            eval_results[results_key]["sae_class"] = "trained"
        else:
            raise ValueError("Please add the correct key for the eval results")
        # Original
...
def plot_2var_graph(
    ...
    trainer_colors: dict[str, str] | None = None,
    return_fig: bool = False,
    connect_points: bool = True,  # New parameter to control line connections
):
    if not trainer_markers:
        trainer_markers = TRAINER_MARKERS
