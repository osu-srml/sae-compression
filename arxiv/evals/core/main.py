    ...
    for sae_release, sae_object_or_id in tqdm(
        ...
        if current_model_str != sae.cfg.model_name:
            ...
            try:
                ...
                # current_model.load_state_dict(torch.load("/home/gupte.31/COLM/sae-compression/gemma2b/pruned/gemma-2-2b_wanda.pth"))
                current_model.load_state_dict(torch.load("/home/gupte.31/COLM/sae-compression/gpt2-small/pruned/gpt2-small_wanda.pth"))
        ...
        try:
            ...
            print("Everyting fine until here")
...
def run_evaluations(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    filtered_saes = [(args.sae_regex_pattern, args.sae_block_pattern)]
