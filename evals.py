...
def multiple_evals(
    ...
    datasets: list[str] = ["Skylion007/openwebtext"],
    ctx_lens: list[int] = [128],
    output_dir: str = "eval_results",
    verbose: bool = False,
    load: bool = False,
    path_to_load_from: str = None
) -> List[Dict[str, Any]]:
    ...
    if current_model_str != sae.cfg.model_name:
        ...
        print(f"Deleting {current_model_str} and loading model {sae.cfg.model_name}")
    ...
    if load:
        current_model.load_state_dict(torch.load(path_to_load_from))
        print(f'Loded model from {path_to_load_from}, {current_model_str}')
    ...
    for ctx_len in ctx_lens:
        for dataset in datasets:
            ...
            # eval_metrics["unique_id"] = f"{sae.cfg.}-{sae_id}"
...
def process_results(
    ...
    for result in cleaned_results:
        json_filename = f"{result['eval_cfg']['context_size']}_{result['eval_cfg']['dataset']}.json".replace(
            "/", "_"
        )
